...
## Getting expected columns from the model's transformer
expected_cols = model.named_steps['preprocessor'].get_feature_names_out()

## Ensuring input_bankdata has all expected columns
for col in expected_cols:
    if col not in input_bankdata.columns:
        input_bankdata[col] = 0  # or a default/fallback value

## Reordering to match model's training data
input_bankdata = input_bankdata[expected_cols]
